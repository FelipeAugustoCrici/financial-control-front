...
const SummarySection = ({ people }: { people: Person[] }) => {
  ...
  const formattedDate = watchedValues.date
    ? new Date(watchedValues.date).toLocaleDateString('pt-BR', {
        month: 'long',
        year: 'numeric',
      })
    : '-';
  ...
  const saveRecordMutation = useMutation({
    mutationFn: async (data: RegistrationFormData) => {
      ...
      if (data.type === 'salary' || data.type === 'income') {
        ...
        if (recordId) {
          let type: 'salaries' | 'extras' | 'incomes' | 'expenses';
          if (editRecord.description === 'Salário' || editRecord.salaryId) type = 'salaries';
          else if (editRecord.sourceId || editRecord.type === 'fixed') type = 'incomes';
          else type = 'extras';
        ...
        if (recordId) {
          return financeService.updateRecord('expenses', recordId, payload);
        }
  ...
  return (
    <FormProvider {...methods}>
      <form onSubmit={methods.handleSubmit(onSubmit)} className="max-w-4xl mx-auto space-y-8">
        ...
        {success && (
          <div className="bg-success-50 border border-success-200 text-success-700 p-4 rounded-xl flex items-center gap-3 animate-in fade-in slide-in-from-top-4">
        ...
        {error && (
          <div className="bg-danger-50 border border-danger-200 text-danger-700 p-4 rounded-xl flex items-center gap-3 animate-in fade-in slide-in-from-top-4">
            <AlertCircle size={24} />
            <div>
            ...
            <Card title="Família e Responsável" className="border-t-4 border-t-primary-600">
              ...
              {families.length === 0 && (
                <p className="mt-4 text-sm text-danger-500 bg-danger-50 p-3 rounded-lg border border-danger-100">
                  Nenhuma família cadastrada. Por favor, cadastre uma família na tela de "Famílias"
                  antes de realizar lançamentos.
                </p>
              )}
            </Card>
